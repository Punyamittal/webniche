...
export const ArtificialHero = ({
  ...
  return (
    <div ref={containerRef} className="artificial-hero-container">
      {/* Navigation */}
      {showNav && (
        <nav className="artificial-hero-nav">
          <div className="artificial-hero-logo">
            <div className="artificial-hero-logo-inner" />
          </div>

          <div className="artificial-hero-nav-links">
            <a href="#" className="artificial-hero-nav-link">
              Creative Journey
            </a>
            <a href="#" className="artificial-hero-nav-link">
              About
          ...
          <div className="artificial-hero-connect">
            + Connect
          </div>
        </nav>
      )}
      ...
      {/* Large text */}
      <div
        className="artificial-hero-large-text"
        style={{
          transform: `translateY(${scrollProgress * 100}px)`,
          opacity: Math.max(0, 1 - scrollProgress * 1.5),
        }}
      >
        <div className="artificial-hero-title">
          {title}
        </div>
      </div>
      ...
      {/* Left side text */}
      <div
        className="artificial-hero-left-text"
        style={{
          transform: `translateX(${-scrollProgress * 200}px)`,
          opacity: Math.max(0, 1 - scrollProgress * 2),
        }}
      >
        <div className="artificial-hero-side-text">
          {leftText.split('\n').map((line, i) => (
      ...
      {/* Right side text */}
      <div
        className="artificial-hero-right-text"
        style={{
          transform: `translateX(${scrollProgress * 200}px)`,
          opacity: Math.max(0, 1 - scrollProgress * 2),
        }}
      >
        <div className="artificial-hero-side-text artificial-hero-side-text-right">
          {rightText.split('\n').map((line, i) => (
            <React.Fragment key={i}>
              {line}
              {i < rightText.split('\n').length - 1 && <br />}
      ...
      {/* Bottom text */}
      <div
        className="artificial-hero-bottom-text"
        style={{
          transform: `translateY(${scrollProgress * 50}px)`,
          opacity: Math.max(0, 1 - scrollProgress * 1.5),
        }}
      >
        <div className="artificial-hero-credit">
          {bottomText}
        </div>
      </div>

      {/* Canvas Container */}
      <div className="artificial-hero-canvas-container">
        <canvas
          ref={canvasRef}
          className="artificial-hero-canvas"
        />
        {/* Film Grain Overlay Canvas */}
        <canvas
          ref={grainCanvasRef}
          className="artificial-hero-grain-canvas"
        />
      </div>
    </div>
  );
