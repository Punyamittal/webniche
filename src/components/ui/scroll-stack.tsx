...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    // Initialize card styles helper function
    const initializeCardStyles = (cardElements: HTMLElement[]) => {
    ...
    if (cards.length === 0) {
      // If cards not found yet, try again after a short delay (for window scroll)
      if (useWindowScroll) {
        const timeout = setTimeout(() => {
          const retryCards = Array.from(document.querySelectorAll('.scroll-stack-card')) as HTMLElement[];
          if (retryCards.length > 0) {
            cardsRef.current = retryCards;
            initializeCardStyles(retryCards);
            setupLenis();
